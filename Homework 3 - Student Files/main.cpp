  ...
  void carefully_move_grocery_items( std::size_t quantity, std::stack<GroceryItem> & broken_cart, std::stack<GroceryItem> & working_cart, std::stack<GroceryItem> & spare_cart )
  {
    ///////////////////////// TO-DO (1) //////////////////////////////
      ...
      else
      {
        ...
        working_cart.push( std::move( broken_cart.top() ) );
        broken_cart.pop();
        trace( working_cart, broken_cart, spare_cart );
...
int main( int argc, char * argv[] )
{
  try
  {
    ...
    {
      const GroceryItem & item   = checkoutCounter.front();
      GroceryItem * dItem = worldWideDatabase.find( item.upcCode() );

      if( dItem != nullptr )
      {
        amountDue += dItem->price();
        std::cout << *dItem << '\n';
      }
      else
      {
