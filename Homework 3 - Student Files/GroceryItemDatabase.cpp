...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  while( fin >> item )
  {
    _database.push_back( std::move( item ) );
  }
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
{
  return find( upc, 0 );    // kicks off the recursion !!
}

GroceryItem * GroceryItemDatabase::find( const std::string & upc, std::size_t index )
{
  // defining base case here
  if( index >= _database.size() )
    return nullptr;

  // next do the visit part
  if( _database[index].upcCode() == upc )
    return &_database[index];

  // finally implement the recurse part
