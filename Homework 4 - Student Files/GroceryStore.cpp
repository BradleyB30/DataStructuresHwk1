...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  while( fin >> std::quoted( upc ) >> quantity ) _inventoryDB[upc] = quantity;
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  for (auto && [upc, groceryItem] : shoppingCart)
  {
    auto fullItemDescription = worldWideGroceryDatabase.find( upc );
    if (fullItemDescription == nullptr)
    {
      receipt << '"' << upc << "\" (" << groceryItem.productName() << ") not found, the item is free!\n";
    }

    else
    {
      receipt << *fullItemDescription << '\n';

      amountDue += fullItemDescription->price();

      auto stockedItem = _inventoryDB.find( upc );
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  for(auto && upc : todaysSales)
  {
    auto inventoryRecord = _inventoryDB.find( upc );

    if(inventoryRecord == _inventoryDB.end() || inventoryRecord->second < REORDER_THRESHOLD)
    {
      reorderReport << '\n'
                    << std::setw( 2 ) << ++lineItemCount << ":  ";

      auto itemFullDescription = worldWideGroceryDatabase.find( upc );
      if (itemFullDescription == nullptr)
      {
        reorderReport << '"' << upc << "\" ";
      }
      ...
      {
        reorderReport << '{' << *itemFullDescription << "} ";
      }
