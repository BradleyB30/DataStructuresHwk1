...
int main()
{
  try
  {
    ...
    const GroceryList proteinList = { { "turkey" }, { "chicken" }, { "steak" } };   // Buying food to meal plan
    const GroceryList veggieList  = { { "corn" }, { "salad" }, { "carrots" } };
    const GroceryList dessertList = { { "cake" }, { "ice cream" }, { "pie" } };

    std::cout << "proteinList: " << proteinList << "\n";
    std::cout << "veggieList: " << veggieList << "\n";
    std::cout << "dessertList: " << dessertList << "\n\n";

    GroceryList shoppingList = proteinList;     // Creating new list and combining lists into shopping list
    shoppingList += veggieList;
    shoppingList += dessertList;

    std::cout << "Combined into shopping list:\n"
              << shoppingList << "\n\n";

    shoppingList.insert( { "ham" }, GroceryList::Position::TOP );
    shoppingList.insert( { "eggs" }, GroceryList::Position::BOTTOM );
    shoppingList.insert( { "cheese" }, shoppingList.find( { "corn" } ) );
    shoppingList.insert( { "napkins" }, 4 );
    shoppingList += { { "tea" }, { "soda" } };    // Also can't forget the drinks

    shoppingList.remove( { "ice cream" } ); // Too much money and sugar need to remove this
    shoppingList.remove(0);
    shoppingList.remove( shoppingList.size() - 1 );

    shoppingList.moveToTop( { "corn" } ); // Corn has been my favorite food since I can remember it goes to the top

    std::cout << "Updated final shopping list after manipulation:\n"
              << shoppingList << "\n\n";

    const GroceryList snackList = { { "chips" }, { "chocolate" }, { "popcorn" } }; // Snacks are a must!

    auto compare = shoppingList <=> snackList;  // comparing shopping list with snack list

    if (compare < 0) {
      std::cout << "Shopping list is less than the snack list \n";
    } else if (compare == 0) {
      std::cout << "Shopping list is equivalent to the snack list \n";
    } else if (compare > 0) {
      std::cout << "Shopping list is greater than the snack list \n";
    }

    auto comp = shoppingList <=> proteinList;  // comparing shopping list with protein list

    if( comp < 0 ) {
      std::cout << "Shopping list is less than the protein list \n";
    } else if ( comp == 0 ) {
      std::cout << "Shopping list is equivalent to the protein list \n";
    } else if( comp > 0 ) {
      std::cout << "Shopping list is greater than the protein list \n";
    }
