...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  stream >> std::quoted( productName ) >> delimiter >> std::quoted( brandName ) >> delimiter >> std::quoted( upcCode ) >> delimiter >> price;

  if( stream )
  {
    GroceryItem obj( std::move( upcCode ), std::move( brandName ), std::move( productName ), price );
    groceryItem = std::move( obj );
  }
  return stream;
...
std::ostream & operator<<( std::ostream & stream, const GroceryItem & groceryItem )
{
  ...
  stream << std::quoted( groceryItem.upcCode() ) << delimeter
         << std::quoted( groceryItem.brandName() ) << delimeter
         << std::quoted( groceryItem.productName() ) << delimeter
         << groceryItem.price();
  return stream;
  /////////////////////// END-TO-DO (22) ////////////////////////////
}
